...
# online
def LGB_CV(train_x, train_y, test_x,res):
    print("cross validation")
    clf = lgb.LGBMClassifier(
        boosting_type='gbdt', num_leaves=31, reg_alpha=0.0, reg_lambda=2,
        max_depth=-1, n_estimators=3100, objective='binary',
        subsample=0.8, colsample_bytree=0.7, subsample_freq=1,
        learning_rate=0.06, min_child_weight=50, random_state=2018, n_jobs=-1
    )
    data = pd.concat([train_x, train_y])
    kf = KFold(n_splits=3,shuffle=False)
    for train_index , test_index in kf.split(data):
        i = 0
        clf.fit(train_index, test_index, eval_set=[(trai_index, test_index)], eval_metric='auc', verbose=100, early_stopping_rounds=2000)
        res['score'] = clf.predict_proba(test_x)[:,1]
        res['score'] = res['score'].apply(lambda x: float('%.8f' % x))
        res.to_csv('submission'+'i'+'.csv', index=False)
        i += 1
...
def LGB_predict(train_x,train_y,test_x,res):
    ...
    return clf
model=LGB_CV(train_x,train_y,test_x,res)
